...
<script>
	function Check() {
		if (Buy.b_email.value.indexOf("@") + "" == "-1"
				|| Buy.b_email.value.indexOf(".") + "" == "-1"
				|| Buy.b_email.value == "") {
			alert("E-mail을 정확히 입력하세요.('@', '.' 포함)")
			Buy.b_email.focus();
			return false;
		}
		if (Buy.b_address.value.length < 1) {
			alert("주소를 입력하세요.")
			Buy.b_address.focus();
			return false;
		}
...
<body>
	<%
	String m_id = (String) session.getAttribute("logID");
	...
	try {
		...
		pstmtp = connp.prepareStatement(strSQL);
		pstmtp.setInt(1, Integer.parseInt(p_num));
		rsp = pstmtp.executeQuery();

		if (rsp.next()) {
			String b_num = rsp.getString("p_num");
			String b_img = rsp.getString("p_img");
			String b_cname = rsp.getString("p_name");
			String b_pay = rsp.getString("p_pay");
			java.sql.Date b_day = rsp.getDate("p_day");
			java.util.Date utilDate = new java.util.Date(b_day.getTime());
			java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd");
			String formattedDate = sdf.format(utilDate);
			String b_keep = rsp.getString("p_keep");
			...
			pstmtm.setString(1, m_id);
			rsm = pstmtm.executeQuery();

			if (rsm.next()) {
		String b_id = rsm.getString("id");
	...
	<div class="center">
		...
		<form name="Buy" method="post" action="Cart_BuyPro.jsp">
